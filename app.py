...
sysprompt = """
You are 'VoteBuddy' – a friendly and knowledgeable helper for anything related to Indian elections.
Answer ONLY in {difflang[lang_code]['name']} language.
...
def getsysprompt(lang_code: str) -> str:
    """Merge the base prompt with the language-specific instructions."""
    config = difflang.get(lang_code, difflang['hi'])
...
def chat():
    ...
    if lang_code not in difflang:
        lang_code = 'hi'

    chat_history = session.get("chat_history", [])
    messages_for_api = [
        {"role": "system", "content": getsysprompt(lang_code)},
        *chat_history,
...
def reset_chat():
    """Delete the saved conversation for the current session."""
